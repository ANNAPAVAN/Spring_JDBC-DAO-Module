...
public class EmployeeDaoImpl implements EmployeeDao {
	...
	public String add(Employee emp) {
		String status = "";
		try {
			List<Employee> empList = JdbcTemplate.query("select * from employee where eno = ?",new Object[] {emp.getEno()}, new EmployeeRowMapper());
			if(empList.isEmpty())
			{
	...
	public Employee search(int eno) {
		Employee emp = null;
		try {
			List<Employee> empList = JdbcTemplate.query("select * from Employee where eno= ? ",new Object[] {eno},new EmployeeRowMapper() );
			if(empList.isEmpty())
			{
				emp = null;
			}else {
